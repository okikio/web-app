...
var gulpNewer = _interopDefault(require('gulp-newer'));
...
var ignoreLog = ["CIRCULAR_DEPENDENCY", "UNRESOLVED_IMPORT", 'EXTERNAL_DEPENDENCY', 'THIS_IS_UNDEFINED'];

var onwarn = function onwarn(_ref, warn) {
  var loc = _ref.loc,
  ...
  if (loc) {
    warn("".concat(loc.file, " (").concat(loc.line, ":").concat(loc.column, ") ").concat(message));
    if (frame) warn(frame);
  } else warn(message);
};
  ...
  return streamList([].concat(_toConsumableArray(["modern"].concat(!dev$1 ? "general" : []).map(function (type) {
    ...
    return ['src/js/app.js', {
      ...
      gulpBetterRollup({
        ...
        onwarn: onwarn
      }, gen ? 'umd' : 'es'), // Minify the file
      debug ? null : gulpTerser(assign$1(minifyOpts, gen ? {
        ie8: true,
        ecma: 5
      } : {
        ecma: 8
      })), gulpRename("app".concat(suffix, ".min.js")), // Rename
      write.apply(void 0, srcMapsWrite) // Put sourcemap in public folder
      ],
      dest: "".concat(publicDest, "/js") // Output

    }];
  })), [dev$1 && staticSite$1 ? null : [['src/js/@(app.vendor|polyfill).js'], {
    opts: {
      allowEmpty: true
    },
    pipes: [gulpNewer("".concat(publicDest, "/js")), // Check for changes
